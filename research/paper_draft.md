...
The empirical work corroborates the theory: across **6 forecasting datasets × 4 N × 3 seeds + 50 RCA cells + 5 UCR datasets × 30 cells + 3 UEA multivariate datasets**, our final architectures achieve (i) **statistically indistinguishable performance from Chronos-2** on forecasting MAE and CRPS (Wilcoxon $p$=0.32; 0W/1L/23T over 24 cells), confirmed by three independent mechanisms (memory consensus, entropy gate, learned abstain head) that all converge to the same mean — direct evidence for the saturation prediction; (ii) **+40 pp R1 over an unstructured LLM-direct baseline on RCA**, with an honestly-reported $-37$ pp loss to a competent rule-based baseline and a stackable mitigation chain (architectural abstain head + dataset prior + LLM upgrade) that reaches **100% out-of-taxonomy detection** on glm-4-plus; (iii) **parity with Rocket on the saturated UCR-5 benchmark** for the TSC router (86.91% vs 87.53%, −0.62 pp; an apparent +0.89 pp was a data-leakage artifact, since corrected — the leakage audit is itself a methodological contribution), with a learned-belief follow-up (§4.11) establishing this as a **cross-task, cross-domain saturation law** — no learned router beats Rocket on the full UCR-22 or multivariate UEA-14 archives (Rocket is the per-cell oracle on 71%/75% of cells; an early 3-dataset "DTW > Rocket on UEA" reading is a small-sample artifact that does not survive the full archive); and (iv) a **learned-margin head** that beats the heuristic gate by $+0.49$ pp on leave-one-dataset-out CV.
...
| **Few-shot TSC** (30 cells, UCR) | router among {Rocket, MOMENT, DTW, …} | Rocket-alone | B7v3 (router + memory, leakage-fixed) | **−0.62pp (parity)** |
...
1. **The TSFM Saturation Hypothesis** — a precise statement of when an LLM-Agent cannot improve over its base model in expectation (Cov(B, D) → 1 ⇒ E[Δ] → 0, Var[Δ] > 0). The hypothesis predicts the average-zero, niche-positive shape of every empirical progression in this paper, is corroborated by Wilcoxon $p$ = 0.32 on the forecasting wrapper-vs-Chronos-2 comparison, and — via a dedicated learned-belief ladder (§4.11) — is elevated to a **cross-task, cross-domain empirical law**: a *learned* router with a stronger TSFM representation and 2.3× more meta-supervision still cannot beat Rocket, and an honestly-calibrated gate converges to never-deviate because Rocket is the per-cell oracle on 71% of univariate (UCR) and 75% of multivariate (UEA) cells.
...
5. **Reasoning tasks**: on RCA, structured Agent achieves +40 pp R1 over an unstructured LLM-direct baseline but $-37$ pp R1 against a competent rule-based baseline — an honest negative finding that motivates the abstain-head architectural fix. The fix reaches 76% out-of-taxonomy recall alone, and **100% when stacked with a stronger LLM and a dataset prior** (§4.7.3). On TSC classification, a learned margin head beats the heuristic gate by +0.49 pp on leave-one-dataset-out CV, and a memory-augmented router reaches **parity** with Rocket on UCR-5 (86.91% vs 87.53%, −0.62 pp) once two data-leakage paths are fixed — the pre-fix +0.89 pp was a leakage artifact (+1.51 pp inflation; method3 M9). The leakage audit, not a "beating SOTA" claim, is the contribution. A follow-up learned-belief ladder (§4.11) confirms that *no* router variant — learned $b(M\mid z)$, MOMENT representation, 2.3× more meta-data, calibrated deviation gate — beats Rocket on the full UCR-22 or multivariate UEA archives; the once-cited "DTW > Rocket on UEA" is a three-dataset small-sample artifact that vanishes on the full archive.
...
- **Classification (§4.5, §4.6)**: Direct B6 Agent loses to Rocket by $-33$ pp ($p<0.001$). After the v8→v10 analog progression and a data-leakage fix (method3 M9), B7v3 reaches $-0.62$ pp on UCR-5 (86.91% vs 87.53%; the pre-fix $+0.89$ pp was a leakage artifact) and $0.0$ pp on less-saturated extended UCR. The expected gain on a satured benchmark is statistically indistinguishable from zero.
- **Classification, strengthened (§4.11)**: replacing the hand-tuned router with a *learned* belief $b(M\mid z)$, scaling the meta-supervision (10→22 datasets), upgrading the representation (MOMENT embedding), and adding an honestly-calibrated deviation gate all fail to beat Rocket; the calibrated gate converges to **zero deviations** because Rocket is itself the oracle on **71% of UCR cells** (oracle gap 1.88 pp). The identical pattern holds on **multivariate UEA** (Rocket oracle on 75% of cells, gap 2.34 pp). TSH is thus corroborated as a **cross-task, cross-domain law**, not a forecasting-only hypothesis.
...
### 4.5 Task B Router · Progression and Final Result (−0.62pp UCR-5 post-leakage-fix, +0.49pp via Learned Margin)

> ⚠️ **ERRATUM (2026-05-30, supersedes the +0.89pp claim below — see `finish3.md` §8 / F-R8.7).**
> The B7v3 **88.42% / +0.89pp** figure was produced by a **data-leaking memory bank**: the
> stored per-classifier accuracies (`all_clf_accs`) and the case's `best_classifier` were
> **test-set** accuracies, and the evaluated cell's own case sat in the bank (self-membership).
> After fixing both leaks — voting on **CV-estimated** accuracies only and **leave-one-cell-out**
> retrieval — B7v3 scores **86.91%, i.e. −0.62pp vs Rocket-alone (87.53%)**. The leak inflated the
> result by **+1.51pp**, which exceeds the entire claimed +0.89pp gain. **The "beats Rocket" claim
> is retracted: on this 30-cell few-shot setting the de-leaked memory-augmented router does not beat
> Rocket-alone.** Root cause (F-R8.8): at N_per_class ≤ 5, CV accuracy is a high-variance proxy for
> test accuracy (CV-winner frequently ≠ test-winner; many cells saturate CV to 1.000), so honest
> memory voting pulls the router off the Rocket default toward CV-lucky but test-worse classifiers.
> The numbers in the table below are the **(leaked) historical** values, retained only for the audit
> trail; treat the erratum figures as authoritative.
...
**UEA Multivariate complementary finding (preliminary, later superseded by §4.11).** As an early probe of the saturation hypothesis, we first tested only 3 UEA multivariate datasets (BasicMotions, ERing, AtrialFibrillation) with multivariate-adapted DTW (channel-wise sum), Euclidean (channel-flattened), and Rocket (native sktime). On this small subset DTW appeared to overtake Rocket (DTW 72.5% vs Rocket 68.3%), which we initially read as evidence that multivariate input offers a larger routing space. **This reading does not survive scaling to the full archive.** §4.11 re-runs the analysis on **14 UEA datasets (81 cells)** and finds the multivariate case is *equally* Rocket-saturated as UCR: Rocket is the per-cell oracle on **75%** of cells, top in 11/14 datasets, with an oracle gap of only 2.34 pp; the apparent DTW advantage is confined to exactly the three small datasets above (AtrialFibrillation / FingerMovements / NATOPS) and is a small-sample artifact. We retain this paragraph as an honest record of a hypothesis we tested and refuted, and direct the reader to §4.11 for the authoritative multivariate result.
...
### 4.11 Learned-Belief Routing and the Saturation Law (Round 9)

§4.5 routed with a hand-tuned factor energy and a memory-consensus gate. A natural objection — raised by an external review of our method document — is that this energy is an *interpretable scoring function*, not a *learned belief*: the components (Availability, CRPS, Regime, …) are hand-specified, so the system can never exceed the designer's priors. We therefore replaced the entire decision core with a **learned belief model** $b(M\mid z)=\mathrm{softmax}(g_\phi(z))$ trained by cross-entropy toward the per-cell oracle winner, and ran a controlled ladder of five honest experiments under **leave-one-dataset-out (LODO)** — strictly stronger than the leave-one-cell-out protocol of §4.5, since an entire held-out dataset never enters training. The purpose was not to add a module but to test *whether any learned router can beat the strong base (Rocket) on standard classification benchmarks*. The answer is a clean, repeated **no**, and the reason elevates the TSH from a forecasting hypothesis to a **cross-task, cross-domain empirical law**.

**The ladder (all LODO, all honest):**

| Variant | Representation $z$ | Decision | UCR-22 vs Rocket | Finding |
|---|---|---|---|---|
| M10 learned belief | 30-d hand features | argmax + belief-margin gate | **−0.80 to −2.90 pp** | learned belief is not a silver bullet |
| M10b learned belief | **512-d MOMENT embedding** | same | **−3.96 pp** (worse) | a stronger representation is *not* the bottleneck |
| M10c calibrated gate | hand features (bagged ×25) | learned $P(\text{deviation correct})$ gate | **−0.45 pp** (10 ds) → **+0.00 pp** (22 ds) | the honestly-calibrated gate learns to *never deviate* |
| M11 (multivariate) | per-channel hand features | argmax | **UEA −4.83 pp** | saturation holds on multivariate data too |

**Three sharpening results.** (i) *Belief strength is an inverse indicator* — when the learned router deviates from Rocket, its mean confidence is **higher when wrong (0.79) than when right (0.51)**; the CE head's high-confidence directions correspond to spuriously over-represented training regions that fail out-of-distribution. (ii) *Scaling the meta-supervision does not help* — expanding the oracle library from 10 to 22 datasets (56 → 128 cells) made the raw router *worse* and drove the calibrated gate to **exactly zero deviations**. The diagnosis is decisive: **Rocket is itself the oracle on 71% of UCR cells** (top in 17/22 datasets, oracle gap only 1.88 pp). The benchmark is *Rocket-saturated*, not expert-switching; an optimal router on such a distribution **is** the constant "use Rocket". (iii) *The saturation is domain-invariant* — repeating the experiment on 14 **multivariate UEA** datasets (81 cells) gives an essentially identical picture: Rocket is the oracle on **75%** of cells (top in 11/14, gap 2.34 pp), and learned routing loses −4.83 pp. The widely-cited "DTW > Rocket on UEA" is a three-dataset small-sample artifact (AtrialFibrillation / FingerMovements / NATOPS); on the full archive it vanishes.

**Interpretation.** This is the exact classification mirror of the forecasting result (§4.8): just as no wrapper beats Chronos-2 because Chronos-2 already covers the forecasting test distribution, no router beats Rocket because **Rocket already covers the UCR/UEA classification distribution**. The calibrated gate's "never deviate" verdict is not a failure of the router; it is the *correct decision* under saturation, and the strongest possible empirical confirmation of the Meta-Decision reduction (§3.0.2): $m^\*=\mathcal{B}$ for the overwhelming majority of $x$. We therefore restate TSH in its general form (§3.0.1) and the synthesis (§5.3.2). Data: `research/results/{m10_learned_belief,m10b_moment_belief,m10c_selective_gate,m11_uea_belief,taskb_expand_ucr}.jsonl`; method in `method4.md`, full findings F-R9.1–9.8 in `finish4.md`.

### 4.12 The Decision Mechanism: Two Questions Need Two Signals (Round 11–12)

Given that no router beats the base on saturated benchmarks (§4.11), we shifted the object of study from *performance* to **the decision mechanism itself**: not "which model to pick" but **"when should a selective agent act, when should it defer, and on what signal does it trust its own judgment?"** This reframing — a *design science of selective action under a strong default* — yields the paper's most transferable mechanistic results, all under LODO.

**Decomposing "confidence" into four variables.** Prior meta-learning work conflates a single *confidence*. We separate it into: **saturation** $\hat g(z)$ (is there any headroom?), **proposal** (who nominates a deviation candidate?), **trust** $\approx P(\text{action safe})$ (will deviating *hurt*?), and **gain** $\approx \mathbb E[\Delta\text{reward}]$ (will deviating *pay*?). The central finding is that these are **distinct random variables**, not one quantity.

**Trust is a risk estimator, not a utility estimator.** A learned belief's raw strength is an *inverse* indicator of correctness (inversion, §4.11); a $K{=}20$ bagged-ensemble belief removes the inversion (conf-when-correct $0.70 >$ wrong $0.625$). A **conformal trust** signal then ranks "will this deviation be *safe*" at **AUC 0.80** on the library-unsaturated 10-classifier benchmark — but the *same* trust ranks "will it *profit*" at only AUC 0.40. Trust answers *safety*, not *utility*.

**Expanding the library un-saturates the benchmark.** Growing the classifier pool from 5 to 10 (adding MiniRocket, WEASEL, Catch22, Mantis) drops Rocket-is-oracle from **71% to 38%** and raises the oracle gap from 1.88 to **5.87 pp** — proving the saturation of §4.11 is a *small-pool artifact*, not intrinsic to UCR. This gave the decision experiments real headroom to study.

**Two questions, two signals (the key result).** Even on the un-saturated library, we could not learn a usable *gain* model: regressing per-cell gain gives correlation 0.40 but profit-classification AUC 0.47 (classification), and only 0.18 on forecasting *despite a 17% relative-MAE oracle gap* — per-cell winner/gain is **unlearnable across both saturated and unsaturated, classification and forecasting** domains. However, decomposing the trust signal reveals that **"avoid harm" and "seek gain" are driven by different signals**: conformal nonconformity ranks *safety* best (AUC 0.79) but gain worst (0.40), whereas **predicted saturation** ranks *gain* best (AUC 0.71) but safety worst (0.35); a naive logistic fusion *dilutes* both. The actionable decision rule is therefore **decoupled**, not a single trust: *deviate iff (conformal says safe) AND (saturation says headroom)*.

**Two engineering corollaries.** (a) The value of an LLM "second opinion" (a structured DeepSeek deferral, precision 0.267 at proposing profitable deviations, 2–3× a trust gate) is the *proposal* role, and it is **replicable by a small learned proposal network** (profit-precision 0.235, no API). (b) Portfolio diversification (weighted ensembles) and exploratory deviation both fail offline on saturated pools (−0.14 to −3.6 pp; exploration net-negative), so they require genuinely complementary pools / online feedback respectively — flagged as future work.

**Deployment.** The conformal *trust* gate transfers directly into our multi-agent system (§ test): adding a CV-saturation-penalized trust gate to the router converts the classification result from −0.42 pp to **exact base parity (+0.00 pp), eliminating two harmful deviations at zero cost** — the empirically-proven optimum (selective abstention) made operational. Data: `research/results/{m13_trust_vs_confidence,m16_gain_model,m17_forecast_gain,m18_portfolio,m19_proposal_net,m20_meta_trust,m21_explore}.jsonl`; methods in `method6.md`/`method7.md`, findings F-R11.1–11.9 / F-R12.1–12.6 in `finish6.md`/`finish7.md`.
...
| **Final result** | 0W/1L/23T MAE, 0% CRPS gap | **−0.62pp** parity (the +0.89pp was a leakage artifact, retracted) |
...
| **TSC UCR-5 Router B7v3** | Rocket 87.5% | B7v3 86.91% (leakage-fixed) | **−0.62pp** | parity (leak artifact retracted) |
...
| TSC UEA multivariate (3-ds partial, superseded) | Rocket 68.3% | DTW 72.5% | DTW > Rocket | small-sample artifact (see below) |
| **TSC UEA multivariate (full 14-ds, §4.11)** | **Rocket 72.5%** | learned belief 67.7% | **−4.83pp** | Rocket-saturated (oracle on 75% cells) |
...
2. **Routing/wrapper architecture achieves parity, never systematic gain on saturated benchmarks** (forecasting v11 parity; TSC B7v3 −0.62pp ≈ parity after the leakage fix; and §4.11's learned-belief ladder confirms no router beats Rocket on full UCR-22 / UEA-14, where the calibrated gate optimally collapses to never-deviate)
...
**The complement: when the honest router is no router.** §4.11 supplies the necessary counterweight to "no method dominates". On *saturated* benchmarks — where one strong base (Chronos-2 for forecasting, Rocket for UCR/UEA classification) is already the per-cell oracle on 70–75% of cells with an oracle gap under 2.5 pp — the honestly-calibrated router learns to **never deviate**, and that is the *correct* policy. We deliberately ran the full learned-belief ladder (learned $b(M\mid z)$, MOMENT representation, 2.3× more meta-data, calibrated deviation gate, multivariate UEA) and every rung confirmed it. The two facts are not in tension: "no method dominates" is about the *cell-level variance* of the winner, while saturation is about its *mean* — under TSH the variance is positive (routing is in principle useful) but the expectation is zero (no router realizes that value without out-of-distribution expert-switching, which standard UCR/UEA archives do not provide). The honest contribution is therefore **a falsifiable boundary**: routing pays only when the base is *not* the oracle, and we show empirically — across two tasks and both univariate and multivariate domains — that on today's standard benchmarks it usually is. Finding genuine routing headroom requires leaving these saturated archives (industrial blind-tests, online-feedback deployment), which we flag as the concrete future direction.

---

## 6. Conclusion

We presented an empirical applicability boundary for LLM-Agent architectures in the time-series foundation-model era, evaluated across three task types: forecasting (where wrappers cannot beat Chronos-2, but a guaranteed-parity wrapper exists), prediction-failure root-cause analysis (where the Agent achieves +40pp over an unstructured baseline), and few-shot classification (where the Agent loses to Rocket as a direct classifier and, as a router over {Rocket, MOMENT, DTW, …}, **reaches parity with Rocket-alone — 86.91% vs 87.53%, −0.62pp — after we corrected a data-leakage artifact that had shown an illusory +0.89pp**).
...
Finally, a learned-belief follow-up (§4.11) sharpens the central claim into a **cross-task, cross-domain saturation law**: a router whose belief $b(M\mid z)$ is *learned* end-to-end (rather than hand-specified), given a stronger TSFM representation and 2.3× more meta-supervision, still cannot beat Rocket on UCR — and an honestly-calibrated deviation gate converges to *never deviating*, because Rocket is the per-cell oracle on 71% of UCR and 75% of multivariate UEA cells. This is the precise classification analogue of the forecasting result, and it means the value of an LLM-Agent meta-controller in the strong-base era is **selective abstention with calibrated confidence**, not accuracy gains on saturated academic benchmarks. We consider this honest negative — that on standard archives the optimal router is often *no* router — among the paper's more useful contributions, and we identify out-of-distribution, expert-switching deployments as where routing should finally pay.

A final study of the **decision mechanism** (§4.12) decomposes the monolithic "confidence" of meta-learning into four distinct variables (saturation / proposal / trust / gain) and establishes our most transferable mechanistic claim: **two questions need two signals**. *Trust* (best estimated by conformal nonconformity, AUC 0.80) answers whether a deviation is *safe*; it does **not** answer whether it will *profit* — a separate, largely unlearnable quantity for which *predicted saturation* is the better-but-still-weak signal (AUC 0.71 vs trust's 0.40). Per-cell gain is unlearnable across saturated/unsaturated and classification/forecasting alike, even with a 17% oracle gap; the actionable rule is therefore the **decoupled** *deviate iff conformal-safe AND saturation-headroom*, not a single trust or gain model. This trust gate, deployed in our multi-agent system, converts classification from −0.42 pp to exact base parity at zero cost. We take the broader lesson to be that a selective-action agent's honest, operational value lies in **risk-controlled abstention with task-appropriate signals**, and that collapsing safety and utility into one confidence — as most prior routers do — is the core design error this series identifies and corrects.
...
|---|---|---|---|
| B7v3 Router | −0.62 pp UCR-5 (parity; +0.89pp was a leakage artifact) | ~54× slower than Rocket | not recommended for raw-acc deployment |
...
- B7v3's apparent UCR-5 gain (pre-leakage-fix +0.89pp; honest −0.62pp) was concentrated on BeetleFly/BirdChicken image-outline morphology where MOMENT pretraining dominates; on less-saturated extended UCR (§4.6 task #42) routing is essentially inactive (B7v3 ≈ Rocket-alone).
