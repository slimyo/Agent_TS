"""#72 · 校准版 B7v3 sweep（isotonic CV→E[test] 校准后 inv-loss 投票）。

与 taskb_router_v3_honest_sweep.py 唯一差异：
  vote_method="inv_loss" + use_cv_calibration=True
校准曲线用 leave-one-cell-out 拟合（剔除查询 cell 自身的 (cv,test) 对），诚实无泄漏。
对照基线 = results/taskb_router_v3_honest_ucr.jsonl（86.91% / -0.62pp）。

输出：results/taskb_router_v3_calibrated_ucr.jsonl
"""
...
def main():
    out = Path("research/results/taskb_router_v3_calibrated_ucr.jsonl")
        ...
        for n in N_PER_CLASS:
            for seed in SEEDS:
                ...
                chosen, y_pred, trace = classification_planner(
                    ...
                    vote_method="inv_loss",          # #72: inv-loss vote …
                    use_cv_calibration=True,          # … with isotonic CV→test calibration
    ...
    fh.close()
    print(f"\nCALIBRATED B7v3 mean acc = {sum(accs)/len(accs)*100:.2f}%  ({len(accs)} cells)",
          flush=True)
